...
var sc_code_ver="h.25.3|01.17.2013" //SiteCatalyst code version
...
s.charSet=s.prop72=s.getCharSet();
if(!s.currencyCode)s.currencyCode="USD";

/* Form Analysis Config */
s.formList="AddBank,Billing,billing_form,form1,ismForm,newbfform,multiplesubmitform,buttonDesignerForm,EditSMI,signup_form,pw_form"
...
s.siteID=window.location.hostname;
s.siteID=s.siteID.replace(new RegExp(/^www\./i),""); //remove www prefix
s.defaultPage="";s.queryVarsList="cmd" //keep cmd query parameter
s.pathExcludeDelim=";";s.pathConcatDelim="";s.pathExcludeList="";
if((s.siteID=="paypal.co.uk") ||(s.siteID=="paypal.com.html") ||(s.siteID=="paypal.com.mx") ||(s.siteID=="paypal.com.sg") ||(s.siteID=="paypal.com.hk") ||(window.location.host.indexOf("www.stage")>-1)) {s.fpCookieDomainPeriods="3";}
...
function s_doPlugins(s) {
  ...
  /* Scrubbed Page URL */
	s.prop36=s.getPageUrl();

  /* Page Name */
	if(!s.pageName)s.pageName=s.prop1?s.prop1:s.prop36;
	s.pageName=s.pageName.toLowerCase();
	s.prop47='D=pageName';
	if(!s.eVar31)s.eVar31=s.pageName;
	if(s.eVar31)s.eVar31=s.getValOnce(s.eVar31,"v31",0);

  /* Previous PageName */
	s.prop43=s.getPreviousValue(s.pageName,'gpv_c43','');

  /* Percent of Page Viewed */
	if(s.prop43)s.prop54=s.getPercentPageViewed();

  /* External Campaigns */
	if(!s.campaign)s.campaign=s.getQueryParam('ext_cid');
	if(!s.campaign)s.campaign=s.getQueryParam('mplx');
	if(s.campaign)s.campaign=s.getValOnce(s.campaign,"v0",0);
	
  /* Channel Manager: Paid & Natural Keywords + Engine */
	s.channelManager('ext_cid,mplx');
	if(s._channel=="Natural Search"){
		s.eVar40=s._keywords;
		s.eVar43=s._partner;}
	else if(s._channel=="Paid Search"){
		s.eVar41=s._keywords;
		s.eVar43=s._partner;}

  /* Email User ID */
	if(!s.prop6)s.prop6=s.getQueryParam('userID');

  /* External Campaign Stacking: 30 day exp, 5 value max, no dupes */
	s.eVar32=s.crossVisitParticipation(s.campaign,'s_ev32','30','5','>','',0);

  /* Campaign Pathing */
	s.prop39=s.campaign?('D=v0+"|"+D=pageName'):('D=pageName');

  /* Internal Campaigns */
	if(!s.eVar15)s.eVar15=s.getQueryParam('intl_cid');
	if(s.eVar15)s.eVar15=s.getValOnce(s.eVar15,"v15",0);

  /* Internal Campaign Stacking: 30 day exp, 5 value max, no dupes */
	s.eVar33=s.crossVisitParticipation(s.eVar15,'s_ev33','30','5','>','',0);

  /* Dedupe Merchant Banner ID */
	if(s.eVar30)s.eVar30=s.getValOnce(s.eVar30,"v30",0);

  /* Form Analysis */
	s.setupFormAnalysis();

  /* Document Title */
	s.prop17=document.title;
	if(s.prop17.length>99){s.prop17=s.prop17.substring(0,96)+"..."}

  /* Cap Form Error Value to 100 Chars */
	if(s.prop14 && s.prop14.length>99)s.prop14=s.prop14.substring(0,99);

  /* Track Refresh */
  	if(s.isPageView){
		s.prop19=s.trackRefresh(s.pageName,'tr_p1');
		s.isPageView=false;
	}
	
  /* Event Flow Deduplication */
	var s_f19,s_f17a,s_fev,s_fadd,s_f17b,s_f23,s_f17c;
	s_fev=s.getPreviousValue(s.events,'gpv_events','');
	if(s.events){
	  if(s.events.indexOf('event19') !=-1){ //A1
		s_f19=s.getValOnce('event19','s_f19',0);
		if(!s_f19)s.events=s.rfl(s.events,"event19"); }
	  if(s.eVar18 && s.events.indexOf('event17') !=-1){ //A2
		s_f17a=s.getValOnce('event17','s_f17a',0);
		if(!s_f17a)s.events=s.rfl(s.events,"event17"); }
	  if(s.events.indexOf('event18') !=-1){ //A3
		if(s_fev.indexOf('event18') !=-1)s.events=s.rfl(s.events,"event18");
		s.c_w('s_f19','0',0);
		s.c_w('s_f17a','0',0); }

	  if(s.events.indexOf('scAdd') !=-1){ //B1
		s_fadd=s.getValOnce('scAdd','s_fadd',0);
		if(!s_fadd)s.events=s.rfl(s.events,"scAdd"); }
	  if(s.eVar8 && s.events.indexOf('event17') !=-1){ //B2
		s_f17b=s.getValOnce('event17','s_f17b',0);
		if(!s_f17b)s.events=s.rfl(s.events,"event17"); }
	  if(s.events.indexOf('scCheckout') !=-1){ //B3
		if(s_fev.indexOf('scCheckout') !=-1)s.events=s.rfl(s.events,"scCheckout");
		s.c_w('s_fadd','0',0);
		s.c_w('s_f17b','0',0); }

	  if(s.events.indexOf('event23') !=-1){ //C1
		s_f23=s.getValOnce('event23','s_f23',0);
		if(!s_f23)s.events=s.rfl(s.events,"event23"); }
	  if(s.eVar6 && s.events.indexOf('event17') !=-1){ //C2
		s_f17c=s.getValOnce('event17','s_f17c',0);
		if(!s_f17c)s.events=s.rfl(s.events,"event17"); }
	  if(s.events.indexOf('event24') !=-1){ //C3
		if(s_fev.indexOf('event24') !=-1)s.events=s.rfl(s.events,"event24");
		s.c_w('s_f23','0',0);
		s.c_w('s_f17c','0',0); }
	}

  /* Internal Search */
	s.eVar20=s.getValOnce(s.eVar20,'v20',0);
	if(s.eVar20){s.prop41='D=v20';s.events=s.apl(s.events,'event3',',',2);}

  /* Kenshoo */
	if(s.getQueryParam('k_clickid') != "") s.eVar37=s.getQueryParam('k_clickid');
	if(s.getQueryParam('k_trackinid')!="") s.eVar38=s.getQueryParam('k_trackingid');

  /* Optimost */
	if(typeof(s)=='object' && typeof(opCreativeId)!='undefined'){
		if(typeof(s.prop37)=='undefined' || s.prop37==null || s.prop37=='::'){
			s.prop37='::' + opCreativeId;
		} else {
			s.prop37+=opCreativeId;
		}
		if(typeof(s.pageName)!='undefined' && s.pageName!=null)
			s.prop25 = s.pageName + ":" + s.prop37;
		else
			s.prop25 = "undefined:" + s.prop37;
		s.eVar25=s.prop25;
	}

  /* Var Clean-up */
	if(s.channel)s.channel=s.channel.toLowerCase();
	s.prop1=s.prop1?s.prop1:"unknown";
	s.prop7=s.prop7?s.prop7.toLowerCase():"none";
	s.prop8=s.prop8?s.prop8.toLowerCase():"none";
	s.prop9=s.prop9?s.prop9.toLowerCase():"none";
	if(!s.eVar19)s.eVar19="D=c7";
  ...
  /* Code Version */
	s.prop53=sc_code_ver; //defined: line 3
	
/* Engagement Manager */
	s.engagementTracker({
        attribute:'class',													//attribute to use to indicate trackable element
		query_searchable:'scQuery',											//class value prefix to indicate element as trackable
		link_searchable:['scTrack','scExit'],
        page_searchable:'scTrackPage',
        cookie_searchable:'scCookie',
		linkType:'o',
        attribute:'class',
        serialize:false,
        defaultHref:'/',
        ...
        extraVars:function(obj){
        	if(obj && obj.searchable) {
        		...
        		if(obj.searchable == 'scTrack')
        		    ...
        		    s.linkType = 'e';
        		s.linkTrackVars += ',prop43,prop26,prop27,eVar35';	
				
				if(navigator&&navigator && navigator.userAgent.indexOf("Safari")>-1 && navigator.userAgent.indexOf("Chrome")==-1){
					var tm=new Date;
					var rd=Math&&Math.random?Math.floor(Math.random()*10000000000000):tm.getTime();
				...
				s.linkName = undefined;
				
				if(!obj.linkName) {
					obj.linkName = 'No Name'
					if(obj.objectClicked) 
						obj.linkName = obj.objectClicked + ' - ' + obj.linkName;
				}
				if(obj.objectClicked && obj.objectClicked.tagName) {
					if(obj.objectClicked.tagName.toLowerCase() == 'a' || obj.objectClicked.tagName.toLowerCase() == 'input') {
						if(obj.objectClicked && obj.objectClicked.type !='text') {
							s.c_w(this.cookieName, obj.linkName.toString() + '^' + s.pageName);
						}
					}
				}
        	}					 
...
/************************** PLUGINS SECTION *************************/
//engagement manager v3.2 
s.engagementTracker = function(v){
	var s = this, o = s.clt_a(v);
	if (v.serialize) {
		s.serializePage(new Object, s, true);
	}
	if (!o) {
		o = s.eo ? s.eo : s.lnk;
		v.cookieCheck();
	...
	if(o.tagName == 'A')
		s.linkName = s.linkName ? s.linkName : (o.name ? o.name : o.innerHTML);
	else
		s.linkName = s.linkName ? s.linkName : (o.name ? o.name : o.value);
		
	var h = o.href ? o.href : v.defaultHref, i = h.indexOf('?'), identifier = atr = o[v.attribute], type = '';
	h = s.linkLeaveQueryString || i < 0 ? h : h.substring(0, i);

	var link = false;
	for(var i = 0; i < v.link_searchable.length; i++) {
		if(atr && (atr.indexOf(v.link_searchable[i]) > -1)) {
			link = true;
		}
	}
	
	if (atr && ((atr.indexOf(v.page_searchable) > -1) || link || (atr.indexOf(v.query_searchable) > -1))) {
		var p = atr.indexOf(':');
		atr = atr.substring(p + 1, atr.length);
		p = atr.indexOf(' ');
		p = p < 0 ? atr.length : p;
		atr = atr.substring(0,p);
		
		identifier = identifier.substring(0,identifier.indexOf(':'));
			
		for(var i = 0; i < v.link_searchable.length; i++) {
			if(o[v.attribute].indexOf(v.link_searchable[i] + ':') > -1) {
				s.linkName = atr;
				s.linkType = s.lt_a(o);
				s.lnk = o;
				s.eo = o;
		...
		if (o[v.attribute].indexOf(v.page_searchable + ':') > -1) {
			s.lnk = undefined;
			s.linkType = undefined;
			s.linkName = undefined;
			s.eo = 0;
...
s.lt_a = function(o){
	...
	if(o.tagName!='A')
		return'o';
	var a=s.linkInternalFilters.split(',');
	for(var i=0;i<a.length;i++){
		if(a[i]&&o.href.indexOf(a[i])>-1){
			return 'o'
		}
	}
	return 'e';
...
+"de);return 0;");
s.serializePage=new Function("t","f","b",""
...
+"ePage(this,t);};}return t;");
s.scTrack=new Function("n","v","f",""
+"var s=this;if(v){s[v]=s.pageName;s.linkTrackVars=v;}!f||f();s.tl('/"
+"','o',n);");
s.scTrackPage=new Function("f",""
+"var s=this,t=s.serializePage(new Object,s);s.linkTrackVars='None';!"
+"f||f();s.t();s=s.serializePage(s,t);");

  //channelManager v2.5 - Tracking External Traffic
s.channelManager= function(a,b,c,d,e,f){
	var s=this,A,B,g,l,m,M,p,q,P,h,k,u,S,i,O,T,j,r,t,D,E,F,G,H,N,U,v=0,X,Y,W,n=new Date;
	...
	if(!O&&!h){
		...
		U=g.indexOf('//');
		q=U>-1?U+2:0;
		Y=g.indexOf('https://www.paypal.com/',q);
		r=Y>-1?Y:i;
			...
			for(G=0;G<E.length;G++){
				...
				if(H>-1){
					...
					for(k=0;k<z.length;k++){
						l=s.getQueryParam(z[k],'',g).toLowerCase();
						if(l){
							M=l;
							if(D[2])N=u=D[2];
							else N=t;
...
+" v==k?'':v");
  ...
  //Function - read/write combined cookies v 0.3
...
+"m:dl});if(ce)s.c_w(cn,'');return r;");
  ...
  //getPageUrl - renamed getPageName v2.1
